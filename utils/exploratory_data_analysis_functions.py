...
def plot_numerical_feature_distribution(data, feature, target_col="target", ax=None):
    ...
    if no_default.empty or default.empty:
        ax.text(
            0.5,
            0.5,
        ...
        ax.set_title(
            f"{feature.replace('_', ' ').title()}", fontweight="bold", fontsize=12
        )
        ax.set_ylabel("Density")
        ax.grid(True, alpha=0.3)
        return {
            "feature": feature,
            "no_default_mean": np.nan,
            "no_default_std": np.nan,
            "default_mean": np.nan,
            "default_std": np.nan,
            "cohens_d": 0.0,
            "effect_size": "Unknown",
    ...
    x_max = max(no_default.quantile(0.95), default.quantile(0.95))  # Remove outliers
    if x_max <= x_min:
        x_max = x_min + 1
...
def plot_categorical_feature_distribution(data, feature, target_col="target", ax=None):
    ...
    for _, (bar, rate, count) in enumerate(zip(bars, rates, counts)):
...
def create_comprehensive_bivariate_analysis(
    ...
    if total_features == 0:
        print("No numerical or categorical features were provided.")
        return {
            "numerical_stats": [],
            "categorical_stats": [],
