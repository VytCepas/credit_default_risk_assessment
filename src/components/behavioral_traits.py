...
class BehavioralTraitsDisplay:
    """
    Display behavioral traits analysis results with visual summaries.
    """
    ...
    def __init__(self):
        ...
        self.trait_descriptions = {
            "job_stability": "Consistency of employment and income over time",
            "payment_behavior": "Signals related to credit use and repayment habits",
            "responsibility": "Indicators connected to household stability and assets",
        }
    ...
    def display_behavioral_traits(self, traits_results: Dict[str, Any]):
        ...
        """
        traits_results = traits_results or {}
        ...
        payment_behavior = self._coerce_score(traits_results.get("payment_behavior", 0))
        responsibility = self._coerce_score(traits_results.get("responsibility", 0))
        overall_score = self._coerce_score(
            traits_results.get("overall_behavioral_score", 0)
        )
    ...
    def _display_overall_score(self, overall_score: float):
        ...
        overall_score = max(0.0, min(100.0, self._coerce_score(overall_score)))
    ...
    def _display_individual_traits(
        self, job_stability: float, payment_behavior: float, responsibility: float
    ):
        """Display individual trait scores"""

        job_stability = max(0.0, min(100.0, self._coerce_score(job_stability)))
        payment_behavior = max(0.0, min(100.0, self._coerce_score(payment_behavior)))
        responsibility = max(0.0, min(100.0, self._coerce_score(responsibility)))
    ...
    def _display_behavioral_insights(self, traits_results: Dict[str, Any]):
        ...
        if job_stability >= 60:
            insights.append(
                "✅ Strong employment stability indicates reliable income patterns"
            )
        else:
            insights.append("⚠️ Employment stability shows room for improvement")
            recommendations.append(
                "Consider building longer employment history or stable income sources"
            )

        if payment_behavior >= 60:
            insights.append(
                "✅ Positive payment behavior patterns suggest good credit management"
            )
        else:
            insights.append(
                "⚠️ Payment behavior indicates potential credit risk factors"
            )
            recommendations.append(
                "Focus on consistent payment history and credit utilization management"
            )

        if responsibility >= 60:
            insights.append(
                "✅ High responsibility indicators suggest reliable financial behavior"
            )
        else:
            insights.append("⚠️ Responsibility metrics show areas for strengthening")
            recommendations.append(
                "Building assets and stable family/housing situations can improve this score"
            )

        if overall_score >= 70:
            insights.append(
                "🎯 Overall profile suggests excellent behavioral traits for lending"
            )
        elif overall_score >= 50:
            insights.append(
                "🎯 Overall profile shows good behavioral traits with minor areas for improvement"
            )
        else:
            insights.append(
                "🎯 Overall profile indicates significant opportunity for behavioral improvement"
            )
        ...
        if recommendations:
            ...
            st.success(
                "No major behavioral warning signals were identified in the current response set."
            )
