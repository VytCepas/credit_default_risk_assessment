...
def load_risk_predictor(model_path: str):
    """Load and cache the trained risk prediction model."""
    try:
...
def get_available_models():
    """Return the risk prediction model artifacts that are present on disk.

    Behavioral traits models are handled separately by behavioral_predictor.py.
    """
...
def predict_with_explanations(model, client_responses):
    ...
    try:
        if model is None:
            raise ValueError("A loaded RiskModel instance is required")
        ...
        if "shap_explanations" in result and result["shap_explanations"]:
            ...
            shap_values = np.array(
                [item.get("contribution", 0.0) for item in shap_explanations]
            )
            processed_features = [
                item.get("feature", "Unknown") for item in shap_explanations
            ]
