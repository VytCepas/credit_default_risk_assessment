        ...
        if isinstance(X, dict):
            ...
            for q_key, value in X.items():
                if q_key in self.questionnaire_mapping:
                    ...
                    try:
                        converted_value = transformer(value)
                        if feature_name == "loan_annuity" and converted_value in (
                            0,
                            0.0,
                            None,
                        ):
...
class RiskModel:
    ...
    def predict(self, client_data: dict) -> dict:
        ...
        expected_order = self._expected_questionnaire_order()
    ...
    def predict_with_explanations(self, client_data: dict, top_n: int = 10) -> dict:
        ...
        if self.shap_explainer is not None:
            try:
                ...
                feature_names = self.feature_names or expected_order
    ...
    def save(self, filepath: str = "src/assets/risk_model.pkl"):
        if not self.is_trained:
    ...
    def load(self, filepath: str = "../src/assets/risk_model.pkl"):
